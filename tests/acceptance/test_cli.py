...
TEST_RESOURCES_FOLDER = pathlib.Path(__file__).resolve().parent.parent / "resources"
...
class TestCli:
    ...
    @pytest.mark.parametrize("use_shell", (True, False))
    def test_tesserae_profalignment_with_cli(self, tmpdir, capfd, use_shell):
        out_file_path = pathlib.Path(tmpdir) / "test_tesserae_alignment_with_cli.sam"
