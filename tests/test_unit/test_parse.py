...
class CortexGraphParser(object):
    ...
    def _read_header(self):
        ...
        if magic_word != CORTEX_MAGIC_WORD:
            raise CortexGraphParserException("Saw magic word {} but was expecting {}".format(magic_word, CORTEX_MAGIC_WORD))

        header = unpack('IIII', self.fh.read(16))

        self.version = header[0]
        if self.version != CORTEX_VERSION:
            raise CortexGraphParserException("Saw version {} but was expecting {}".format(self.version, CORTEX_VERSION))

        self.kmer_size = header[1]
        if self.kmer_size <= 0:
            raise CortexGraphParserException("Saw kmer size {} but was expecting value >= 0".format(self.kmer_size))

        self.kmer_bits = header[2]
        if self.kmer_bits <= 0:
            raise CortexGraphParserException("Saw kmer bits {} but was expecting value >= 0".format(self.kmer_size))

        self.num_colors = header[3]
        if self.num_colors <= 0:
            raise CortexGraphParserException("Saw number of colors {} but was expecting value >= 0".format(self.kmer_size))
...
class CortexGraphBuilder(object):
    ...
    def build(self):
        return io.BytesIO(self.magic_word + pack('IIII', self.version, self.kmer_size, self.kmer_bits, self.num_colors))
